...
_Generated on 2026-08-21 by `scripts/verify.mjs` (**36 of 36** checks) and, for
...
| ✅ | and both answers point at the same outbox row | `outboxId 1d0263e1-9f32-4aeb-b589-987de00aef05 === 1d0263e1-9f32-4aeb-b589-987de00aef05` |
| ✅ | the database holds exactly one row for that id | `1 row(s) with clientMessageId=verify-idem-1787271194779` |
...
| ✅ | the API still accepts the send instead of rejecting it | `HTTP 202, outboxId=3a11a116-59ac-48b7-9b72-44f3d1e8ed63` |
...
| ✅ | the signature recomputes from the body and the secret | `header sha256=fd6e3091f88f55b8c…  recomputed sha256=fd6e3091f88f55b8c…` |
...
| ✅ | the retry waits instead of hammering | `6034 ms between two arrivals of the same payload` |
...
| | Check | Evidence |
|---|---|---|
| ✅ | a delivery that keeps failing ends in the dead queue | `attempts=3, status=dead, lastError="HTTP 500"` |
| ✅ | it is queryable rather than only logged | `GET /v1/webhooks/deliveries?status=dead returned it by id fe29a0ab` |
| ✅ | replay accepts the dead delivery | `replayed=1` |
| ✅ | and it reaches the receiver on the second life | `2 arrival(s) after the replay` |
...
| ✅ | the drain is underway, with sends in flight | `20 sent, 12 in flight, 40 still to go` |
| ✅ | the replica comes back on its own | `back in 1.4 s after SIGKILL` |
...
| ✅ | and the session sends again on the node that inherited it | `status=sent, engineMessageId=SIM334b178a1` |
